...
def parse(data_path, task_id, word2id=None, bsz=32, DATA_TYPES=['train', 'valid', 'test'], global_sentence_max=0, use_cache=True):
    ...
    for data_type in DATA_TYPES:
        ...
        if os.path.exists(cache_path) and use_cache:
            ...
            filenames = list(filter(lambda x: FORMAT_STR.format(task_id) in x and data_type in x, os.listdir(data_path)))
            if len(filenames) == 0:
